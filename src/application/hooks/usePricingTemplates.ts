...
import type { PricingTemplateListParams } from "@/domain/repositories/IPricingTemplateRepository";
...
export const usePricingTemplateUsages = (id?: string) =>
  useQuery({
    ...
    queryFn: () => pricingTemplateRepository.usages(id as string),
    enabled: Boolean(id),
  });
...
export const usePricingTemplateMutations = () => {
  ...
  return {
    create: useMutation({
      mutationFn: (payload: CreatePricingTemplateRequestDto) => pricingTemplateRepository.create(payload),
      onSuccess: invalidateTemplates,
    }),
    update: useMutation({
      mutationFn: (vars: { id: string; payload: UpdatePricingTemplateRequestDto }) =>
        pricingTemplateRepository.update(vars.id, vars.payload),
      onSuccess: invalidateTemplates,
    }),
    remove: useMutation({
      mutationFn: (id: string) => pricingTemplateRepository.delete(id),
      onSuccess: invalidateTemplates,
    }),
    apply: useMutation({
      mutationFn: (vars: { id: string; payload: ApplyPricingTemplateRequestDto }) =>
        pricingTemplateRepository.apply(vars.id, vars.payload),
    ...
    applyBulk: useMutation({
      mutationFn: (vars: { id: string; payload: ApplyPricingTemplateBulkRequestDto }) =>
        pricingTemplateRepository.applyBulk(vars.id, vars.payload),
      onSuccess: (_data, vars) => {
        invalidateTemplates();
    ...
    saveRuleAsTemplate: useMutation({
      mutationFn: (vars: { ruleId: string; payload: SavePricingRuleAsTemplateRequestDto }) =>
        pricingTemplateRepository.saveRuleAsTemplate(vars.ruleId, vars.payload),
      onSuccess: invalidateTemplates,
    }),
  };
};
