...
import type { ProductPricingRuleDto, UpsertProductPricingRuleRequestDto } from "@/domain/types/productOperations.types";
...
export const useProductPricingRules = (productId?: string) =>
  useQuery({
    ...
    queryFn: () => productRepository.getPricingRules(productId as string),
...
export const useProductPricingRuleMutations = (productId?: string) => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: UpsertProductPricingRuleRequestDto) =>
      productRepository.createPricingRule(productId as string, payload),
    onSuccess: (created) => {
      if (pricingRulesKey) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: UpsertProductPricingRuleRequestDto }) =>
      productRepository.updatePricingRule(id, payload),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => productRepository.deletePricingRule(id),
    onSuccess: (_data, id) => {
      if (pricingRulesKey) {
        queryClient.setQueryData<ProductPricingRuleDto[]>(pricingRulesKey, (current = []) =>
  ...
  const reorderMutation = useMutation({
    mutationFn: (orderedPricingRuleIds: string[]) =>
      productRepository.reorderPricingRules(productId as string, orderedPricingRuleIds),
