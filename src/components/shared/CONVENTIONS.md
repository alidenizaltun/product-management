...
| Liste tablosu | `DataTableServer` (`PageHeader`/`ListPage` + kart toolbar, `EmptyState`, `TableSkeleton`, `nk-tb-col-tools` / `nk-tb-actions` / `btn-trigger`, `DataTablePagination`) | Paylaşılan tablo bir kez kullanılır. İkinci bir tablo kiti yazma. Toolbar arama kartın içindedir. Ürün listesi kart ızgarasıdır, tabloya çevrilmez. |
...
| Liste tablosu yükleniyor | `DataTableServer` `isLoading` → `TableSkeleton` | Elle spinner + `text-soft` "Yükleniyor..." satırı yazma. |
...
| Tam sayfa oluştur/düzenle | `FormPage` | Tek kayıt formu. Kendi `PageHeader`'ını içine alma — çift başlık olur. Ürün bölüm sayfaları `ProductSectionPage` kabuğunda kalır. |
| Tam sayfa kayıt detayı | `DetailPage` + `DetailSection` / `DetailCard`, yüklemede `DetailSkeleton`, hatada `StatusAlert` (`onRetry`) | Boş eşlenen alanlar `"—"`. Ham İngilizce enum adı gösterme. Ürün workspace (`ProductDetailPage` + bölüm sayfaları) bu kabuğa alınmaz. |
...
| Kaydet/Onayla butonu | `LoadingButton` (`loading`, `loadingText`) veya `FormPage` submit | Elle `{isLoading ? <Spinner/>+"..." : "Kaydet"}` deseni yazma; `LoadingButton` ayrıca yüklenirken çift tıklamayı da engeller. |
| Kaydet/Vazgeç düzeni | Birincil aksiyon (Kaydet/Oluştur) görsel olarak öne çıkan konumda (yatay düzende sağda, dikey `d-grid` yığında üstte); ikincil aksiyon (İptal/Vazgeç) daha soluk (`color="light" outline`). Uzun formlarda `FormPage` `stickySave` — yalnızca başlık aksiyonları kaybolacak kadar uzunsa. | Mevcut PM ve B2B formları bu düzeni zaten kullanıyor. |
| Onay istemi (sil, geri alınamaz işlem) | `ConfirmDialog` (`variant="danger"` silme için, `warning` diğer geri dönüşü zor işlemler için) | |
| Modal kapatma | `FormModal` / `DetailModal` / `ConfirmDialog` | Karartılmış alana tıklamak kapatmaz (`backdrop="static"`). İptal, header X ve Escape kapatır. Ham `Modal` kullanılıyorsa aynı prop zorunlu. |
