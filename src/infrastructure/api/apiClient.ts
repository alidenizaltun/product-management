...
class ApiClient {
    ...
    private processQueue(error: unknown): void {
        this.failedQueue.forEach((promiseItem) => {
            if (error) {
    ...
    private async refreshToken(error: AxiosError): Promise<void> {
        ...
        const message =
            response.data.errors?.length > 0
                ...
                : "Token yenileme basarisiz.";

        throw new Error(message);
    }

    private extractErrorMessage(error: AxiosError | any): string {
