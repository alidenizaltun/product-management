...
import { attributeDefinitionsApi } from "@/modules/attributes/api/attributes.api";
import {
...
export const useAttributeDefinitions = () =>
  useQuery({ queryKey: attributeKeys.definitions, queryFn: attributeDefinitionsApi.list });

export const useAttributeDefinition = (id?: string) =>
  useQuery({
    queryKey: id ? attributeKeys.definition(id) : ["attributes", "definitions", "missing"],
    queryFn: () => attributeDefinitionsApi.byId(id as string),
    enabled: Boolean(id),
  });
  ...
  return {
    create: useMutation({
      mutationFn: (payload: CreateProductAttributeDefinitionRequestDto) =>
        attributeDefinitionsApi.create(payload),
    ...
    update: useMutation({
      mutationFn: (vars: { id: string; payload: UpdateProductAttributeDefinitionRequestDto }) =>
        attributeDefinitionsApi.update(vars.id, vars.payload),
      onSuccess: (_d, vars) => {
