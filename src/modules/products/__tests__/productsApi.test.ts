...
describe("productsApi", () => {
 ...
 await productsApi.getProducts({ search: "Test", page: 2, pageSize: 10 });
 ...
 const result = await productsApi.getProductById(mockProductDto.id);
 expect(result.id).toBe("prod-001");
 expect(result.name).toBe("Test Ürünü");
 });

 it("bulunamazsa 404 hatası fırlatır", async () => {
 await expect(productsApi.getProductById("yok-id")).rejects.toMatchObject({
 statusCode: 404,
 });
 });
 });
 ...
 const result = await productsApi.getProductDetail(mockProductDetailDto.id);
 ...
 const result = await productsApi.createFullProduct(payload);
 ...
 await productsApi.createFullProduct({
 ...
 await expect(
 productsApi.updateFullProduct("prod-001", {
 ...
 await productsApi.updateFullProduct("prod-001", {
 product: { productCode: "P", name: "P", kind: 1, status: 0, isActive: true, isSellable: true, isPurchasable: true, trackInventory: true, defaultCurrencyCode: "TRY" },
 prices: [
 { priceType: 1, amount: 100, currencyCode: "TRY" },
 ...
 await expect(productsApi.deleteProduct("prod-001")).resolves.toBeUndefined();
 });
 });
});
