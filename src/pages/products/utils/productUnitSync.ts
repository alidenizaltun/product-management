...
import { buildOfferingPayload } from "@/pages/products/components/pricing/LicenseOfferingFormFields";
import type { ProductFormValues, ProductUnitForm } from "@/pages/products/types/productEditor.types";
import type { CreateProductUnitRequestDto, ProductUnitDto, UnitRole } from "@/domain/types/productOperations.types";
...
export const addOrReuseProductUnit = async ({
    ...
    const created = await productRepository.createProductUnit(productId, payload);
...
const persistOfferingUnitScope = async (
    ...
    await productRepository.updateLicenseOffering(productId, offeringId, buildOfferingPayload(offering));
};
...
export const removeProductUnit = async ({
    ...
    if (unit.id) {
        await productRepository.deleteProductUnit(unit.id);
    }
